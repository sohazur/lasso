...
function CallLog({ flash }: { flash: (m: string, k?: "ok" | "bad") => void }) {
  ...
  async function onTestCall() {
    setPlacing(true);
    try {
      const r = await triggerTestCall();
      flash(`Test call placed: ${r.call_id ?? "?"}`);
      void load();
    } catch (e: unknown) {
      flash("Test call failed: " + (e instanceof Error ? e.message : String(e)), "bad");
    } finally {
      setPlacing(false);
    }
  }
  ...
  return (
    <section className="lasso-card">
      <div className="lasso-row" style={{ alignItems: "center", marginBottom: 14 }}>
        <button className="btn btn-primary" onClick={onTestCall} disabled={placing}>
          {placing ? "Placing…" : "Trigger test call"}
        </button>
        <button className="btn btn-ghost" onClick={load}>Refresh</button>
        <span className="lasso-hint">Auto-refresh every 4 s</span>
      </div>
